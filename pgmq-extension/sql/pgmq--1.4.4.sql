...
-- Overloaded function to read messages with conditional retrieval using jsonb
CREATE FUNCTION pgmq.read(
    queue_name TEXT,
    vt INTEGER,
    qty INTEGER,
...
BEGIN
    ...
    IF conditions IS NOT NULL THEN
        sql := sql || ' AND message @> ' || quote_literal(conditions);
    END IF;

    sql := sql || ' ORDER BY msg_id ASC
                LIMIT $1
                FOR UPDATE SKIP LOCKED
            )
            UPDATE pgmq.' || qtable || ' m
            SET
                vt = clock_timestamp() + ' || quote_literal(make_interval(secs => vt)) || ',
                read_ct = read_ct + 1
            FROM cte
            WHERE m.msg_id = cte.msg_id
            RETURNING m.msg_id, m.read_ct, m.enqueued_at, m.vt, m.message;';
