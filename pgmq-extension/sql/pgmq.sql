...
CREATE FUNCTION pgmq.acquire_queue_lock(queue_name TEXT) RETURNS bigint AS $$
DECLARE
  lock_id bigint;
BEGIN
  lock_id := hashtext('pgmq.queue_' || queue_name);
...
DECLARE
    ...
    lock_id bigint;
BEGIN
    lock_id := pgmq.acquire_queue_lock(queue_name);
    BEGIN
      EXECUTE FORMAT(
          $QUERY$
